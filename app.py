...
.stApp { background-color: #0f0f0f; color: #e8e0d4; }
h1, h2, h3 { font-family: 'Playfair Display', serif; color: #e8e0d4; }
.hero { text-align: center; padding: 2.5rem 0 1.5rem; border-bottom: 1px solid #2a2a2a; margin-bottom: 2rem; }
.hero h1 { font-size: 2.8rem; letter-spacing: -0.5px; margin-bottom: 0.3rem; color: #e8e0d4; }
.hero p { font-size: 1rem; color: #888; font-weight: 300; letter-spacing: 0.5px; }
.accent { color: #c9a96e; }
[data-testid="stFileUploader"] { border: 1.5px dashed #333; border-radius: 12px; padding: 1rem; background: #161616; }
[data-testid="stFileUploader"]:hover { border-color: #c9a96e; }
.stButton > button { background: #c9a96e !important; color: #0f0f0f !important; font-family: 'DM Sans', sans-serif !important; font-weight: 500 !important; border: none !important; border-radius: 8px !important; padding: 0.6rem 2rem !important; font-size: 0.95rem !important; transition: opacity 0.2s !important; width: 100%; }
.stButton > button:hover { opacity: 0.85 !important; }
.stSelectbox > div > div { background: #161616 !important; border: 1px solid #2a2a2a !important; color: #e8e0d4 !important; border-radius: 8px !important; }
.stTextInput > div > div > input { background: #161616 !important; border: 1px solid #2a2a2a !important; color: #e8e0d4 !important; border-radius: 8px !important; }
.stProgress > div > div { background: #c9a96e !important; }
.stAlert { border-radius: 10px !important; border-left: 3px solid #c9a96e !important; background: #161616 !important; color: #e8e0d4 !important; }
.output-box { background: #161616; border: 1px solid #2a2a2a; border-radius: 12px; padding: 1.5rem 1.8rem; margin-top: 1rem; line-height: 1.7; color: #d4c9b8; font-size: 0.95rem; white-space: pre-wrap; max-height: 420px; overflow-y: auto; }
.step-label { display: inline-block; background: #1e1e1e; border: 1px solid #2a2a2a; border-radius: 20px; padding: 0.2rem 0.9rem; font-size: 0.78rem; color: #c9a96e; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 0.6rem; }
hr { border-color: #1e1e1e !important; margin: 1.8rem 0 !important; }
[data-testid="stDownloadButton"] > button { background: #1e1e1e !important; color: #c9a96e !important; border: 1px solid #c9a96e !important; border-radius: 8px !important; font-family: 'DM Sans', sans-serif !important; font-weight: 500 !important; width: 100%; }
[data-testid="stDownloadButton"] > button:hover { background: #c9a96e !important; color: #0f0f0f !important; }
...
<div class="hero">
    ...
    <p>Upload a long MP3 · Get a clear, structured summary · Powered by Whisper API + Claude</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<small style='color:#555'>Built with Streamlit · OpenAI Whisper API · Claude</small>", unsafe_allow_html=True)


# ── Helper functions ───────────────────────────────────────────────────────────

CHUNK_MB = 24  # OpenAI Whisper API limit is 25MB per request
    ...
    if not anthropic_key or not openai_key:
        ...
        if run_btn:
            ...
            try:
                ...
                status_text.markdown(f"**Sending** {len(chunks)} part(s) to Whisper API…")
                progress_bar.progress(0.05)

                transcript = transcribe_with_openai(chunks, openai_key, progress_bar, status_text)
